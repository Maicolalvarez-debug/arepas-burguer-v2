'use client'; import useSWR from 'swr'; import { useRouter } from 'next/navigation'; import { useEffect, useState } from 'react';
const fetcher=(u:string)=>fetch(u).then(r=>r.json());
export default function ProductForm({ id }:{ id?: number }){ const router=useRouter(); const { data: cats }=useSWR('/api/categories', fetcher);
  ...
  const save=async(e:React.FormEvent)=>{ e.preventDefault(); const method=id?'PUT':'POST'; const url=id?`/api/products/${id}`:'/api/products';
    const res=await fetch(url,{ method, headers:{'Content-Type':'application/json'}, body: JSON.stringify(state) }); if(res.ok) router.push('/admin/products'); };
  return (<div className="max-w-xl"><h1 className="text-2xl font-semibold mb-4">{id?'Editar':'Nuevo'} producto</h1>
    <form className="space-y-3" onSubmit={save}>
      <div><label className="label">Nombre</label><input className="input" placeholder="Ej. Clásica" value={state.name||''} onChange={e=>setState({...state, name:e.target.value})}/></div>
      <div><label className="label">Descripción</label><textarea className="input" placeholder="Ingredientes o notas" value={state.description||''} onChange={e=>setState({...state, description:e.target.value})}/></div>
