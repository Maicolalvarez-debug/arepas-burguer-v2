...
async function main(){
  ...
  for (const p of products){ await prisma.product.upsert({ where:{ sku:p.name }, update:{ ...p }, create:{ ...p, sku:p.name } }); }
  const modifiers = [
  ...
  for (const m of modifiers){ await prisma.modifier.create({ data: m }).catch(()=>{}); }
  const allProducts = await prisma.product.findMany(); const allMods = await prisma.modifier.findMany();
  for (const p of allProducts){ for (const m of allMods){
    await prisma.productModifier.upsert({ where:{ productId_modifierId:{ productId:p.id, modifierId:m.id } }, update:{}, create:{ productId:p.id, modifierId:m.id } });
  } }
  console.log('Seed OK');
}
