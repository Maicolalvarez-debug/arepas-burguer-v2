...
function MenuInner(){
  ...
  const sendWhatsApp=()=>{
    ...
    try { fetch('/api/orders', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ tableCode: table || null, items: cart }) }); } catch(e) { console.error(e); }
    try { fetch('/api/orders', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ tableCode: table || null, items: cart }) }); } catch(e) { console.error(e); }
...
{promo && (
  <div className="card bg-white/5 border-white/10 mt-2">
    <div className="flex items-center gap-3">
      {promo.imageUrl && <img src={promo.imageUrl} className="w-24 h-16 object-cover rounded-lg" /> }
      <div className="grow">
        <div className="font-semibold">Promo del día: {promo.title}</div>
        <div className="text-sm opacity-80">{promo.description}</div>
      </div>
      <div className="text-lg font-semibold">${promo.price?.toLocaleString("es-CO")}</div>
    </div>
  </div>
)}

{table && <div className="mb-2 text-sm opacity-80">Pedido para <span className="font-semibold">Mesa {table}</span></div>}
