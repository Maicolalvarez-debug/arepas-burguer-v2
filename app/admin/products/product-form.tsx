...
export default function ProductForm({ id }:{ id?: number }){
  ...
  const [state, setState] = useState<any>({ name:'', description:'', price:0, cost:0, stock:0, active:true, categoryId:null, imageUrl:'' });
  useEffect(()=>{ if (!id) return; (async()=>{ const j = await fetch(`/api/products/${id}`).then(r=>r.json()); setState({ ...j, imageUrl: j.imageUrl || '' }); })(); },[id]);
