...
// app/admin/modifiers/ModifiersClient.tsx
'use client';
...
export default function ModificadorsClient({{ data }}: {{ data: Item[] }}) {{
  ...
  async function saveOrder() {{
    ...
    try {{
      ...
      const res = await fetch('/api/modifiers/reorder', {{
        method: 'POST',
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Modificadors</h1>
        <div className="flex items-center gap-2">
          <Link href="/admin/modifiers/new" className="rounded-xl px-4 py-2 border">Nuevo</Link>
          <button
            type="button"
      ...
      {items.map((x, i) => (
        <div key={x.id} className="flex items-center gap-2 border rounded-lg px-3 py-2">
          ...
          <div className="flex gap-1">
            ...
            <Link href={"/admin/modifiers/" + x.id} className="rounded-lg px-2 py-1 border" title="Editar">Editar</Link>
