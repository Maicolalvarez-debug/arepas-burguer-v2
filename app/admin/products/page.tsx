...
export default function AdminProducts(){
  ...
  const move = async (index:number, dir:'up'|'down')=>{
    const base:any[] = (typeof items !== 'undefined' && Array.isArray(items) ? items as any[]
                      : (typeof data !== 'undefined' && Array.isArray(data) ? data as any[] : []));
    const arr = [...(base||[])];
      ...
      {list.map((p:any)=>(
        <div key={p.id} className="card flex items-center gap-3">
          ...
          <div className="flex gap-2 items-center">
            <button className="btn" onClick={()=>move(((items||data)||[]).findIndex((x:any)=>x.id===p.id),'up')}>↑</button>
            <button className="btn" onClick={()=>move(((items||data)||[]).findIndex((x:any)=>x.id===p.id),'down')}>↓</button>
