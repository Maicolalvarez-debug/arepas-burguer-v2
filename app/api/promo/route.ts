...
export async function GET(){ try:
  const now = new Date();
  const promo = await prisma.dailyPromo.findFirst({ select: { id: true, active: true, startDate: true, endDate: true, title: true, description: true, image: true, imageUrl: true, price: true, cost: true  } catch(err:any){ return NextResponse.json(null); } },
