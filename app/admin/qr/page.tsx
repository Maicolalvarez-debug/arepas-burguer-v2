...
export default function QrPage(){
  ...
  const gen = async (e: React.FormEvent)=>{
    e.preventDefault(); setLoading(true); setSvg('');
    ...
    if(res.ok){ const xml = await res.text(); setSvg(xml); } else { const j = await res.json().catch(()=>({error:'Error'})); alert('Error: '+j.error); }
    setLoading(false);
  };
  return (<div className="space-y-4">
    ...
    <div className="card">{svg ? <div dangerouslySetInnerHTML={{ __html: svg }} /> : <div className="text-sm text-gray-400">Aquí verás los QR. Imprime esta página o guarda la imagen.</div>}</div>
  </div>);
}
