...
export default function Menu(){
  ...
  const sendWhatsApp=()=>{
    ...
    const msg = encodeURIComponent(`Pedido Arepas Burguer:\n${lines.join('\n')}\nTotal: $${total.toLocaleString('es-CO')}`);
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between">
      ...
      <button className="btn" onClick={sendWhatsApp}>Enviar pedido ({cart.length})</button>
    </div>
          ...
          {!loading && selected && (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm opacity-80">Total: <span className="font-bold">${currentModalTotal().toLocaleString('es-CO')}</span></div>
                <button className="btn" onClick={()=>setShowModal(false)}>Cancelar</button>
                <button className="btn-primary" onClick={addFromModal}>Agregar al carrito</button>
              </div>
            </div>
          )}
        </div>
