...
export async function GET(req: NextRequest) {
  ...
  if (from) (where.createdAt as any).gte = new Date(String(from) + 'T00:00:00.000')
  if (to)   (where.createdAt as any).lte = new Date(String(to)   + 'T23:59:59.999')
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { ok: false, error: e?.message || 'order_create_failed' },
