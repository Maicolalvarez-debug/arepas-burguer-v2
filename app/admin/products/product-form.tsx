...
export default function ProductForm({ id }:{ id?: number }){
  ...
  useEffect(()=>{
    if(!id) return;
    (async()=>{
      const j=await fetch(`/api/products/${id}`).then(r=>r.json());
      const current = (j?.modifiers||[]).map((x:any)=> x.modifier?.id ?? x.id);
      setState({ ...j, modifierIds: current });
    })();
  },[id]);
  ...
  const save=async(e:React.FormEvent)=>{
    ...
    if(res.ok) router.push('/admin/products'); else alert('No se pudo guardar');
  };
  ...
  return (<div className="max-w-2xl">
    ...
    <form className="space-y-3" onSubmit={save}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div><label className="label">Precio (COP)</label><input className="input" type="text" inputMode="decimal" value={state.price ?? ''} onChange={e=>setState({...state, price:Number(e.target.value)})}/></div>
        <div><label className="label">Costo (COP)</label><input className="input" type="text" inputMode="decimal" value={state.cost ?? ''} onChange={e=>setState({...state, cost:Number(e.target.value)})}/></div>
